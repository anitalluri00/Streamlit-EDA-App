...
def plot_confusion_matrix(cm, labels):
    ...
    # Convert all labels to str to avoid mathtext parsing problems
...
if uploaded_file:
    ...
    feature_cols = st.multiselect(
        ...
        default=[col for col in all_columns if col != target_col] if len(all_columns) > 1 else []
    )
    ...
    if model_type == "Classification":
        ...
        fig_cm = plot_confusion_matrix(confusion_matrix(y_test, y_pred), labels)
        st.pyplot(fig_cm)
    else:
